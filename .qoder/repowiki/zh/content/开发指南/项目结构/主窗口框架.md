...
**更新** 现已支持跨平台部署，包括 Uno 平台的回退标题栏实现，确保在不同平台上的一致用户体验。

## 项目结构
本项目采用 WinUI 3 的 XAML + C# 组合方式组织 UI 与逻辑，并支持跨平台部署：
...
- 跨平台配置：Agentic.Desktop.csproj（Uno SDK 配置）
...
FRAME["Frame<br/>页面承载"]
TB["TitleBar<br/>Windows 原生"]
FB["Grid TitleBar<br/>Uno 回退"]
end
subgraph "页面"
MP["MainPage<br/>聊天页"]
...
- [MainWindow.xaml:15-73](file://Agentic.Desktop/MainWindow.xaml#L15-L73)
- [MainWindow.xaml.cs:16-41](file://Agentic.Desktop/MainWindow.xaml.cs#L16-L41)
- [MainPage.xaml.cs:16-34](file://Agentic.Desktop/MainPage.xaml.cs#L16-34)
- [SettingsPage.xaml.cs:15-59](file://Agentic.Desktop/SettingsPage.xaml.cs#L15-59)
...
- [MainWindow.xaml:1-101](file://Agentic.Desktop/MainWindow.xaml#L1-L101)
- [MainWindow.xaml.cs:1-118](file://Agentic.Desktop/MainWindow.xaml.cs#L1-L118)
...
- MainWindow：继承自 Window，承载 TitleBar 或自定义 Grid 回退标题栏与 NavigationView，管理页面导航与连接状态显示。
...
**更新** MainWindow 现在支持双标题栏实现：Windows 平台的原生 TitleBar 和 Uno 平台的自定义 Grid 回退标题栏。

章节来源
- [App.xaml.cs:18-85](file://Agentic.Desktop/App.xaml.cs#L18-L85)
- [MainWindow.xaml.cs:10-118](file://Agentic.Desktop/MainWindow.xaml.cs#L10-L118)
...
## 架构总览
下图展示从应用启动到主窗口加载、默认页面导航、以及设置页建立连接后更新全局状态的完整时序，包括跨平台标题栏的选择逻辑。
...
App->>Win : Activate()
Win->>Win : InitializeComponent()<br/>ExtendsContentIntoTitleBar=true
alt WINDOWS 平台
Win->>Win : SetTitleBar(AppTitleBar)<br/>SystemBackdrop = MicaBackdrop
else Uno 平台
Win->>Win : SetTitleBar(AppTitleBarFallback)<br/>隐藏原生 TitleBar
end
Win->>Nav : Loaded 事件
...
**更新** 窗口初始化现在包含条件编译逻辑，根据目标平台选择不同的标题栏实现。

```mermaid
flowchart TD
Start(["应用启动"]) --> CreateWin["创建 MainWindow"]
CreateWin --> InitXaml["InitializeComponent()<br/>设置 TitleBar/图标/尺寸"]
InitXaml --> PlatformCheck{"检查目标平台"}
PlatformCheck --> |WINDOWS| NativeTitlebar["设置原生 TitleBar<br/>SystemBackdrop = MicaBackdrop"]
PlatformCheck --> |Uno| FallbackTitlebar["设置 Grid 回退标题栏<br/>隐藏原生 TitleBar"]
NativeTitlebar --> ShowWin["Activate()"]
FallbackTitlebar --> ShowWin
...
**新增** 主窗口现在支持两种标题栏实现，通过条件编译确保跨平台兼容性：

#### Windows 平台标题栏
- 使用 Microsoft.UI.Xaml.Controls.TitleBar 原生控件
- 支持 SystemBackdrop（Mica 效果）
- 内置返回按钮和侧边栏切换功能
- 自动集成系统任务栏图标

#### Uno 平台回退标题栏
- 使用自定义 Grid 控件模拟标题栏外观
- 手动实现图标、标题和状态指示器
- 启用 NavigationView 的 PaneToggleButtonVisible 属性
- 通过 ExtendsContentIntoTitleBar 避免双重标题栏

```mermaid
classDiagram
class MainWindow {
+AppTitleBar : TitleBar
+AppTitleBarFallback : Grid
...
MainWindow --> TitleBar : "Windows 平台"
MainWindow --> GridTitleBar : "Uno 平台"
```
...
- [MainWindow.xaml:18-73](file://Agentic.Desktop/MainWindow.xaml#L18-L73)
- [MainWindow.xaml.cs:20-35](file://Agentic.Desktop/MainWindow.xaml.cs#L20-L35)
- [Agentic.Desktop.csproj:11-12](file://Agentic.Desktop/Agentic.Desktop.csproj#L11-L12)
...
**更新** 图标设置现在同时适用于 Windows 和 Uno 平台，确保一致的视觉体验。

章节来源
- [MainWindow.xaml:21-28](file://Agentic.Desktop/MainWindow.xaml#L21-L28)
- [MainWindow.xaml.cs:37](file://Agentic.Desktop/MainWindow.xaml.cs#L37)
...
**更新** 回退标题栏使用固定高度（40px）确保在不同平台上的稳定性。

章节来源
- [MainWindow.xaml:13-16](file://Agentic.Desktop/MainWindow.xaml#L13-L16)
- [MainWindow.xaml:43-50](file://Agentic.Desktop/MainWindow.xaml#L43-L50)
...
**更新** 连接状态更新现在支持双标题栏，根据平台自动选择正确的 UI 元素。
...
alt WINDOWS 平台
MW->>MW : 更新 StatusDot 和 StatusText
else Uno 平台
MW->>MW : 更新 StatusDotFallback 和 StatusTextFallback
end
```
...
- [MainPage.xaml.cs:26-34](file://Agentic.Desktop/MainPage.xaml.cs#L26-L34)
- [SettingsPage.xaml.cs:19-59](file://Agentic.Desktop/SettingsPage.xaml.cs#L19-L59)
- [MainWindow.xaml.cs:43-71](file://Agentic.Desktop/MainWindow.xaml.cs#L43-L71)
...
**更新** 在 Uno 平台上，侧边栏切换通过 NavigationView 的 PaneToggleButton 实现。

章节来源
- [MainPage.xaml.cs:49-70](file://Agentic.Desktop/MainPage.xaml.cs#L49-L70)
- [MainWindow.xaml.cs:73-84](file://Agentic.Desktop/MainWindow.xaml.cs#L73-L84)
...
图表来源
- [MainWindow.xaml:75-98](file://Agentic.Desktop/MainWindow.xaml#L75-L98)
- [MainPage.xaml.cs:16-34](file://Agentic.Desktop/MainPage.xaml.cs#L16-L34)
- [SettingsPage.xaml.cs:19-59](file://Agentic.Desktop/SettingsPage.xaml.cs#L19-L59)
- [ChatViewModel.cs:11-47](file://Agentic.Desktop/ViewModels/ChatViewModel.cs#L11-L47)
- [PermissionHandler.cs:26-44](file://Agentic.Desktop/Services/PermissionHandler.cs#L26-L44)
- [Agentic.Desktop.csproj:1](file://Agentic.Desktop/Agentic.Desktop.csproj#L1)
...
- 资源占用：
  - 断开连接时清空消息与引用，降低内存占用
- 跨平台优化：
  - 条件编译减少不必要的代码路径，提高运行时性能

**更新** 跨平台实现通过条件编译避免了不必要的 UI 元素创建，提升了性能。
...
- 连接状态不更新：
  ...
  - 验证平台特定的 UI 元素（StatusDot vs StatusDotFallback）是否正确访问
- 页面无法导航：
...
- 跨平台问题：
  - 验证条件编译符号 WINDOWS 是否正确定义
  - 检查 Uno 平台的回退标题栏是否正确显示

**更新** 新增跨平台故障排查指南，重点关注条件编译和平台特定功能。
...
## 结论
MainWindow 作为 WinUI 3 的主窗口，通过自定义 TitleBar 与 NavigationView 实现了清晰的导航框架与良好的用户体验。结合 App 的全局状态管理与 ViewModel 的数据驱动模式，实现了连接状态同步、页面切换与消息流的高效处理。新增的跨平台支持确保了在 Uno 平台上的稳定运行，通过条件编译和回退机制提供了统一的用户体验。遵循本文提供的最佳实践，可进一步提升稳定性与可维护性。

**更新** 跨平台标题栏实现为应用提供了更好的可移植性，同时保持了 Windows 平台的原生体验。
...
- 跨平台配置：
  - Agentic.Desktop.csproj 中的 Uno SDK 配置和条件编译符号

**更新** 新增跨平台配置说明，帮助开发者理解和配置多目标框架。
