...
## 更新摘要
**所做更改**   
- 更新了 Package.appxmanifest 应用清单部分，添加了 uap:ShowNameOnTiles 配置以改善 Windows 开始菜单和任务栏的磁贴显示效果
- 修正了 XML 声明格式问题
- 增强了磁贴名称显示功能的详细说明
...
- 输出类型与目标框架
  - 输出类型为可执行程序，目标框架为 net10.0-windows10.0.26100 和 net10.0-desktop，支持多目标平台。
  - 根命名空间设置为 Agentic_Desktop。
  - 启用 UnoSingleProject 以支持跨平台开发。

- 平台与运行时标识
  - 条件编译符号 WINDOWS 用于 Windows 特定功能。
  - ApplicationManifest 条件包含 app.manifest 仅针对 Windows 目标。

- WinUI 与 MSIX 工具链
  - 条件启用 MSIX 项目能力以支持单项目 MSIX 打包体验。
  - HasPackageAndPublishMenu 暴露"打包与发布"菜单。
...
- NuGet 包依赖
  ...
  - ShihaoShen.Agentic.ACPLibrary 0.1.0-nightly.1ad856d0d1c865d194a161c6481986a2260b4c50：ACP 客户端库，用于与 Agent 通信。
...
ReadGlobal --> Build["构建与打包<br/>dotnet build/run/publish"]
Build --> End(["结束"])
```
...
### .csproj 项目文件核心配置
- 目标框架与平台
  - TargetFrameworks 指定 net10.0-windows10.0.26100 和 net10.0-desktop 双目标框架。
  - TargetPlatformMinVersion 限制最低系统版本。
...
**已更新** 增强了磁贴显示配置，添加了 uap:ShowNameOnTiles 元素以改善 Windows 开始菜单和任务栏的磁贴显示效果。
...
- **新增** 磁贴名称显示配置
  - uap:DefaultTile 中的 uap:ShowNameOnTiles 元素配置磁贴名称显示行为。
  - uap:ShowOn Tile="square150x150Logo" 和 uap:ShowOn Tile="wide310x150Logo" 分别配置方形和宽形磁贴的名称显示。
  - 此配置确保应用在 Windows 开始菜单和任务栏中正确显示应用名称。
- 权限与功能特性
...
- 集成 WinUI Analyzer 分析器以提供代码质量检查和最佳实践建议。
...
- 锁定 Uno.Sdk 版本为 6.6.29 以确保跨平台构建的一致性。
...
- 业务与工具
  - ShihaoShen.Agentic.ACPLibrary 0.1.0-nightly.1ad856d0d1c865d194a161c6481986a2260b4c50：ACP 协议客户端。
  - Markdig 1.3.2：Markdown 渲染。
...
- 多平台发布（net10.0-windows10.0.26100/net10.0-desktop）需按需选择目标，减少不必要的运行时与依赖。
- 自包含发布可减少目标机缺少运行时导致的额外开销，但会增加安装包大小。
...
- 磁贴名称不显示
  - 确认 Package.appxmanifest 中的 uap:ShowNameOnTiles 配置是否正确。
  - 检查 uap:ShowOn 元素是否包含所需的磁贴尺寸。
- 启动时报 DPI 或兼容性错误
...
章节来源
- [Agentic.Desktop.csproj:76-81](file://Agentic.Desktop/Agentic.Desktop.csproj#L76-L81)
- [Package.appxmanifest:46-51](file://Agentic.Desktop/Package.appxmanifest#L46-L51)
- [app.manifest:5-18](file://Agentic.Desktop/app.manifest#L5-L18)
- [App.xaml.cs:75-80](file://Agentic.Desktop/App.xaml.cs#L75-L80)

## 结论
Agentic.Desktop 的配置围绕 WinUI 3 + Windows App SDK 的现代桌面应用范式展开：通过 .csproj 明确目标框架与平台、启用 MSIX 打包、集中管理 NuGet 依赖；通过 Package.appxmanifest 声明应用标识、资源与权限，并配置磁贴显示行为；通过 app.manifest 控制 UAC 与 DPI 行为；通过 launchSettings.json 灵活切换开发与打包运行模式；通过 Directory.Build.props 和 global.json 统一管理构建规则与 SDK 版本。遵循本文档的配置要点与扩展建议，可高效地定制与交付高质量的桌面应用。
...
### 配置扩展与自定义指南
- 新增目标平台
  - 在 .csproj 的 TargetFrameworks 中添加新平台，并确保相应的运行时支持。
- 调整目标框架或最小系统版本
...
- 自定义磁贴显示行为
  - 在 Package.appxmanifest 的 uap:ShowNameOnTiles 元素中添加新的 uap:ShowOn 配置以支持更多磁贴尺寸。
- 自定义启动参数与环境变量
