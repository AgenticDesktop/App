...
## 更新摘要
**变更内容**   
- 新增 Agentic.Desktop.Tests 测试项目，包含完整的 xUnit 测试基础设施
- 添加四个核心测试套件：ChatMessageTests、FileSystemHandlerTests、MarkdownHelperTests、MockAgentTransportTests
- 更新单元测试和集成测试章节，详细说明测试框架使用方法和最佳实践
- 增强测试覆盖率说明，涵盖消息处理、文件系统操作、Markdown 转换和模拟传输
...
- 性能分析与内存泄漏检测、性能瓶颈识别
- 常见调试场景与问题排查方法
- **新增**：xUnit 测试框架使用和测试套件编写指南

## 项目结构
本项目为 WinUI 3 桌面应用，采用 MVVM 模式，通过 ACP 协议与 Agent 进程通信。**新增测试项目**提供全面的单元测试和集成测试支持。关键目录与职责：
...
- Services/*：终端管理、权限处理、文件系统访问、本地化服务、Markdown 转换
- Mocks/MockAgentTransport.cs：模拟 ACP 传输，用于无真实 Agent 时的 UI 开发与测试
- **.Tests/**：**新增** xUnit 测试项目，包含四个核心测试套件
...
subgraph "测试项目"
TESTS["Agentic.Desktop.Tests"]
CMT["ChatMessageTests"]
FST["FileSystemHandlerTests"]
MDT["MarkdownHelperTests"]
...
- **新增**：Markdown 转换与消息处理
  - MarkdownHelper 提供 Markdown 到 HTML 和纯文本的转换功能
  - ChatMessage 类实现可观察的消息对象，支持流式更新和属性变更通知

章节来源
- [App.xaml.cs:67-71](file://Agentic.Desktop/App.xaml.cs#L67-L71)
- [SettingsViewModel.cs:73-87](file://Agentic.Desktop/ViewModels/SettingsViewModel.cs#L73-87)
- [ChatViewModel.cs:153-206](file://Agentic.Desktop/ViewModels/ChatViewModel.cs#L153-206)
- [TerminalManager.cs:16-68](file://Agentic.Desktop/Services/TerminalManager.cs#L16-L68)
- [PermissionHandler.cs:26-44](file://Agentic.Desktop/Services/PermissionHandler.cs#L26-44)
- [FileSystemHandler.cs:17-30](file://Agentic.Desktop/Services/FileSystemHandler.cs#L17-L30)
- [MarkdownHelper.cs:19-50](file://Agentic.Desktop/Services/MarkdownHelper.cs#L19-L50)
- [ChatMessage.cs:14-31](file://Agentic.Desktop/ViewModels/Messages/ChatMessage.cs#L14-L31)

## 架构总览
下图展示从用户交互到 ACP 协议通信的关键流程，包括 Mock 与真实传输路径，以及测试基础设施的集成。
...
Note over TEST : xUnit 测试套件验证各组件行为
TEST->>TR : 模拟传输测试
TEST->>FS : 文件系统操作测试
TEST->>MD : Markdown 转换测试
TEST->>CM : 消息对象测试
```
...
- [ChatViewModel.cs:153-206](file://Agentic.Desktop/ViewModels/ChatViewModel.cs#L153-206)
- [SettingsViewModel.cs:73-113](file://Agentic.Desktop/ViewModels/SettingsViewModel.cs#L73-113)
- [MockAgentTransport.cs:27-117](file://Agentic.Desktop/Mocks/MockAgentTransport.cs#L27-L117)
- [Agentic.Desktop.Tests.csproj:15-25](file://Agentic.Desktop.Tests/Agentic.Desktop.Tests.csproj#L15-L25)
...
### **新增**：Markdown 转换与消息处理
- MarkdownHelper：提供 Markdown 到 HTML 和纯文本的转换功能
  - ToHtml：使用 Markdig 库将 Markdown 转换为 HTML
  - ToPlainText：移除 Markdown 格式标记，提取纯文本内容
- ChatMessage：可观察的消息对象，支持流式更新
  - 实现 ObservableObject，支持属性变更通知
  - TextContent 和 IsStreaming 属性的双向绑定支持
  - 自动生成唯一 ID 和时间戳
...
TESTS["Agentic.Desktop.Tests"] --> XUNIT["xUnit Framework"]
TESTS --> COVERLET["coverlet.collector"]
TESTS --> PROJ_REF["项目引用"]
```

**图表来源** 
- [SettingsViewModel.cs:73-113](file://Agentic.Desktop/ViewModels/SettingsViewModel.cs#L73-113)
- [ChatViewModel.cs:77-87](file://Agentic.Desktop/ViewModels/ChatViewModel.cs#L77-87)
- [App.xaml.cs:67-71](file://Agentic.Desktop/App.xaml.cs#L67-L71)
- [Agentic.Desktop.csproj:53-60](file://Agentic.Desktop/Agentic.Desktop.csproj#L53-L60)
- [launchSettings.json:1-10](file://Agentic.Desktop/Properties/launchSettings.json#L1-L10)
- [Agentic.Desktop.Tests.csproj:15-29](file://Agentic.Desktop.Tests/Agentic.Desktop.Tests.csproj#L15-L29)
...
- **新增**：测试相关问题
  - 测试用例无法运行：检查 xUnit 测试适配器安装和项目配置
  - 测试执行超时：增加测试超时时间或优化测试逻辑
  - 断言失败：检查测试数据和预期结果的匹配性
...
- 针对常见问题给出排查步骤与修复建议
- **新增**：使用 xUnit 测试框架进行全面的单元测试和集成测试，确保代码质量和稳定性
...
### **新增**：xUnit 测试框架使用指南
- 测试项目配置
  - 使用 Microsoft.NET.Test.Sdk 作为基础测试框架
  - 集成 xunit 和 xunit.runner.visualstudio 用于测试执行和可视化
  - 配置 coverlet.collector 进行代码覆盖率收集
- 测试套件结构
  - ChatMessageTests：测试消息对象的构造函数、属性、事件通知
  - FileSystemHandlerTests：测试文件系统操作的权限验证和文件读写
  - MarkdownHelperTests：测试 Markdown 转换功能的正确性
  - MockAgentTransportTests：测试模拟传输的行为和状态管理
- 测试最佳实践
  - 使用 [Fact] 特性标记测试方法
  - 采用 Arrange-Act-Assert 模式组织测试逻辑
  - 使用 IDisposable 接口清理测试资源
  - 编写异步测试时使用 async/await 模式
...
- 单元测试
  ...
  - **新增**：测试 ChatMessage 的属性变更通知和流式更新机制
  - **新增**：测试 FileSystemHandler 的路径验证和文件操作安全性
  - **新增**：测试 MarkdownHelper 的各种 Markdown 语法转换
- 集成测试
  - 使用真实 StdioAgentTransport 与外部 Agent 进程进行端到端测试
  - 验证 AcpClient 初始化、会话创建、消息发送与取消全流程
  - **新增**：测试完整的应用工作流程，从用户输入到响应显示
- 测试数据与断言
  - 构造标准化 JSON 请求行，断言返回结果结构与字段
  - 验证事件触发顺序与次数（MessageReceived、TransportFaulted）
  - **新增**：使用临时目录和随机文件名确保测试隔离性

章节来源
- [ChatMessageTests.cs:8-101](file://Agentic.Desktop.Tests/ChatMessageTests.cs#L8-L101)
- [FileSystemHandlerTests.cs:24-101](file://Agentic.Desktop.Tests/FileSystemHandlerTests.cs#L24-L101)
- [MarkdownHelperTests.cs:8-100](file://Agentic.Desktop.Tests/MarkdownHelperTests.cs#L8-L100)
- [MockAgentTransportTests.cs:12-136](file://Agentic.Desktop.Tests/MockAgentTransportTests.cs#L12-L136)
...
### **新增**：测试执行与调试
- 测试执行
  - 使用 Visual Studio 测试资源管理器运行单个测试或整个测试套件
  - 使用命令行 `dotnet test` 执行测试
  - 配置测试过滤器按名称、类别或失败状态筛选测试
- 测试调试
  - 在测试方法中设置断点进行单步调试
  - 使用 Assert 断言失败时自动中断执行
  - 查看测试输出和日志信息
- 覆盖率分析
