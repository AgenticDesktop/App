...
## 项目结构
- Services/MarkdownHelper.cs：Markdown 转换的核心服务，封装 Markdig Pipeline 与 ToHtml/ToPlainText 方法。
...
## 核心组件
- MarkdownHelper：静态类，维护一个预构建的 Markdig Pipeline，提供 ToHtml 与 ToPlainText 两个方法。
...
- 预处理：输入字符串校验（空值快速返回）。
- 转换引擎：Markdig Pipeline（启用高级扩展），ToHtml 生成 HTML；ToPlainText 使用正则剥离格式标记。
...
### MarkdownHelper：Markdown 到 HTML/纯文本转换
- 设计要点
  - 使用 MarkdownPipelineBuilder 构建一次性的 Pipeline，并启用高级扩展，避免重复构建开销。
  - ToHtml：对空输入快速返回空串；否则调用 Markdig.Markdown.ToHtml。
  - ToPlainText：通过一系列正则表达式移除常见 Markdown 标记（标题、粗斜体、代码块、行内代码、链接等），作为临时纯文本展示方案。
- 复杂度与性能
  - ToHtml：时间复杂度近似 O(n)，n 为输入长度；空间复杂度取决于生成的 HTML 大小。
  - ToPlainText：多次正则替换，时间复杂度 O(k·n)，k 为替换规则数量；适合短文本与低频率调用。
- 可扩展性
...
ChooseMethod --> |ToHtml| UsePipeline["使用 Markdig Pipeline 转换"]
ChooseMethod --> |ToPlainText| StripRegex["执行正则剥离格式标记"]
...
章节来源
- [MainPage.xaml:18-45](file://Agentic.Desktop/MainPage.xaml#L18-L45)
- [MainPage.xaml.cs:86-94](file://Agentic.Desktop/MainPage.xaml.cs#L86-L94)
...
- [App.xaml.cs:66-73](file://Agentic.Desktop/App.xaml.cs#L66-L73)

## 结论
当前 Markdown 渲染器以 Markdig 为核心，提供稳定的 ToHtml/ToPlainText 能力，并通过静态 Pipeline 提升性能。UI 层暂以原始文本展示，预留 WebView2 集成点以实现富文本渲染。建议在后续迭代中引入 HTML 缓存、安全过滤与样式主题定制，以提升用户体验与安全性。
